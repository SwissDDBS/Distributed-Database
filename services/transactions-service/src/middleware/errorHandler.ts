...
// 2PC specific errors
export const twoPhaseCommitError = (phase: 'PREPARE' | 'COMMIT' | 'ABORT', message: string, details?: Record<string, any>) =>
  createError(`2PC ${phase} failed: ${message}`, 409, `2PC_${phase}_FAILED`, details);

export const coordinatorError = (message: string, details?: Record<string, any>) =>
  createError(`Coordinator error: ${message}`, 500, 'COORDINATOR_ERROR', details);
